...
function vegMarket(input) {
  const vegPricePerKg = Number(input[0]);
  const fruitPricePerKg = Number(input[1]);
  const vegKg = Number(input[2]);
  const fruitKg = Number(input[3]);

  const cost = vegKg * vegPricePerKg + fruitKg * fruitPricePerKg;
  const costInEvro = cost / 1.94;
  console.log(costInEvro.toFixed(2));
}
